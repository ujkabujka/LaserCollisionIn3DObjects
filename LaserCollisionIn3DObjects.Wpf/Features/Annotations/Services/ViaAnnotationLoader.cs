...
public sealed class ViaAnnotationLoader
{
    ...
    private static List<RegionRecord> ReadRegions(JsonElement regions)
    {
        ...
        {
            foreach (var region in regions.EnumerateArray()) if (TryParseRegion(region, out var parsed)) list.Add(parsed);
        }
        else if (regions.ValueKind == JsonValueKind.Object)
        {
            foreach (var property in regions.EnumerateObject()) if (TryParseRegion(property.Value, out var parsed)) list.Add(parsed);
        }
        return list;
    }
    ...
    private static bool TryParseRegion(JsonElement regionElement, out RegionRecord region)
    {
        ...
        TryParseShape(shapeName, attributes, out var shape);
    ...
    private static bool TryParseShape(string name, JsonElement shape, out IAnnotationShape? parsed)
    {
        parsed = null;
