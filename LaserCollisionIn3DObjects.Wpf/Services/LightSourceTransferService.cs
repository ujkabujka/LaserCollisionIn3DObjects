...
public sealed class LightSourceTransferService
{
    private readonly CylindricalRayGenerator _cylinder = new();
    private readonly AxisymmetricRayGenerator _axisymmetric = new();
    public LightSourceTransferData Export(CylindricalLightSourceItemViewModel source)
    {
    ...
    public LightSourceTransferData Export(ProjectedLightSourceItemViewModel source)
    {
    ...
    /// <summary>
    /// Imports portable source-local rays using either their recorded world frame or an identity frame.
    /// Collision file imports use <see cref="ImportedSourcePosePolicy.CanonicalIdentityFrame"/> while
    /// frame-preserving consumers retain the portable file's provenance pose.
    /// </summary>
    public ProjectedLightSourceItemViewModel Import(
        LightSourceTransferData file,
        ImportedSourcePosePolicy posePolicy = ImportedSourcePosePolicy.PreserveFileFrame)
    {
        ArgumentNullException.ThrowIfNull(file);
        var frame = posePolicy switch
        {
            ImportedSourcePosePolicy.PreserveFileFrame => file.Frame,
            ImportedSourcePosePolicy.CanonicalIdentityFrame => new Frame3D(Vector3.Zero, Quaternion.Identity),
            _ => throw new ArgumentOutOfRangeException(nameof(posePolicy), posePolicy, null),
        };
        var x=frame.TransformDirectionToWorld(Vector3.UnitX);var y=frame.TransformDirectionToWorld(Vector3.UnitY);var z=frame.TransformDirectionToWorld(Vector3.UnitZ);
        var source=new ProjectedLightSourceItemViewModel { Name=file.Name, ProfileDefinition=file.Profile, OriginKind=ProjectedLightSourceOriginKind.ImportedTextFile, BaseOrientation=frame.Orientation, SourceFrame=new PointSourceFrameState { Origin=new Point3(frame.Position.X,frame.Position.Y,frame.Position.Z),AxisX=new Vector3D(x.X,x.Y,x.Z),AxisY=new Vector3D(y.X,y.Y,y.Z),AxisZ=new Vector3D(z.X,z.Y,z.Z)}};
        foreach(var ray in file.Rays) source.ExactRays.Add(new Ray3D(frame.TransformPointToWorld(ray.PositionLocal),Vector3.Normalize(frame.TransformDirectionToWorld(ray.DirectionLocal)))); return source;
    }
    private static LightSourceTransferData Create(string name,AxisymmetricSourceProfileDefinition p,Frame3D f,float tilt,Vector3 point,IEnumerable<Ray3D> rays)=>new(name,p,f,tilt,point,rays.Select(r=>new LightSourceTransferRay(f.TransformPointToLocal(r.Origin),Vector3.Normalize(f.TransformDirectionToLocal(r.Direction)))).ToList());
...
public enum ImportedSourcePosePolicy
{
    PreserveFileFrame,
    CanonicalIdentityFrame,
}
