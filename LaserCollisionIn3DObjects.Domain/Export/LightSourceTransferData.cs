...
public sealed record LightSourceTransferData(
    string Name, AxisymmetricSourceProfileDefinition Profile, Frame3D Frame,
    float TiltWeight, Vector3 TiltPointLocal, IReadOnlyList<LightSourceTransferRay> Rays);
...
public static class LightSourceTextSerializer
{
    ...
    public static string Serialize(LightSourceTransferData file)
    {
    ...
    public static LightSourceTransferData Parse(string text)
    {
        ...
        if(rays.Count!=count) throw new FormatException($"Ray count mismatch: header declares {count} rays but {rays.Count} were parsed."); var result=new List<LightSourceTransferRay>(count);for(var n=1;n<=count;n++){if(!rays.TryGetValue(n,out var ray)||ray.p is null||ray.d is null)throw new FormatException($"Ray {n} must contain exactly one Position {n} and Direction {n}.");if(ray.d.Value.LengthSquared()<=0)throw new FormatException($"Invalid Direction {n} at line {ray.l}: direction vector must be non-zero.");result.Add(new(ray.p.Value,Vector3.Normalize(ray.d.Value)));} var file=new LightSourceTransferData(name,profile,frame,tilt,tiltPoint,result);Validate(file);return file;
    }
    private static void Geometry(StringBuilder b,AxisymmetricSourceProfileDefinition p){ if(p.Kind==AxisymmetricSourceKind.Cylinder){Line(b,"RADIUS",p.Radius);Line(b,"LENGTH",p.Length>0?p.Length:p.Height);return;} if(p.Kind!=AxisymmetricSourceKind.Hybrid){Line(b,"RADIUS_START",p.RadiusStart);Line(b,"RADIUS_END",p.RadiusEnd);Line(b,"LENGTH",p.Length);if(p.Kind==AxisymmetricSourceKind.CircularOgive){Line(b,"ARC_RADIUS",p.ArcRadius);Line(b,"CURVATURE_DIRECTION",p.OgiveCurvatureDirection);}return;}Line(b,"SEGMENT_COUNT",p.Hybrid.Count);for(var i=0;i<p.Hybrid.Count;i++){var s=p.Hybrid[i];var n=$"SEGMENT_{i+1}_";Line(b,n+"KIND",s.Kind);Line(b,n+"LENGTH",s.Length);Line(b,n+"RADIUS_START",s.RadiusStart);Line(b,n+"RADIUS_END",s.RadiusEnd);Line(b,n+"ARC_RADIUS",s.ArcRadius?.ToString("R",Culture)??"NONE");Line(b,n+"CURVATURE_DIRECTION",s.OgiveCurvatureDirection);}}
