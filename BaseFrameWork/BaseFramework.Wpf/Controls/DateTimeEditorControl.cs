...
public sealed class DateTimeEditorControl : UserControl
{
    ...
    public DateTimeEditorControl(ObservableObject target, InspectableMemberMetadata member)
    {
        ...
        {
            if (MemberAccess.MatchesPropertyChange(_member, args.PropertyName))
            {
    ...
    private void Commit()
    {
        if (_isUpdating || MemberAccess.IsReadOnly(_member) || !_datePicker.SelectedDate.HasValue)
        {
            return;
        }
        ...
        MemberAccess.SetValue(_member, _target, value);
    }
    ...
    {
        if (MemberAccess.GetValue(_member, _target) is not DateTime value)
        {
            value = DateTime.Now;
        }
