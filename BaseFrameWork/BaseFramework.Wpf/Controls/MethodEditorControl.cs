...
public sealed class MethodEditorControl : UserControl
{
    public MethodEditorControl(
        ...
        IMemberAccessEvaluator? accessEvaluator = null,
        InspectableAccessContext? accessContext = null,
        IInspectorEditorRegistry? editorRegistry = null)
    {
        var effectiveAccessEvaluator = accessEvaluator ?? new DefaultMemberAccessEvaluator();
        var effectiveAccessContext = accessContext ?? InspectableAccessContext.Empty;
        var effectiveEditorRegistry = editorRegistry ?? new DefaultInspectorEditorRegistry();
        ...
        var invokeButton = new Button
        {
            ...
            HorizontalAlignment = HorizontalAlignment.Left,
            IsEnabled = MemberAccess.CanInvoke(member)
        };
        ...
        if (hasParameters)
        {
            ...
            toggle!.Expanded += (_, _) =>
            {
                ...
                {
                    var build = BuildParameterEditors(member.Parameters!, provider, effectiveAccessEvaluator, effectiveAccessContext, effectiveEditorRegistry);
        ...
        {
            if (!MemberAccess.CanInvoke(member))
            {
                return;
            }
            ...
            try
            {
                if (!hasParameters)
                {
                    MemberAccess.Invoke(member, target, Array.Empty<object?>());
                    return;
                }
                ...
                MemberAccess.Invoke(member, target, values);
            }
            catch (Exception ex)
            {
    ...
    private static (StackPanel Panel, List<Func<object?>> Editors) BuildParameterEditors(
        IReadOnlyList<InspectableMemberMetadata> parameters,
        IObjectMetadataProvider provider,
        ...
        IInspectorEditorRegistry editorRegistry)
    {
        ...
        {
            var editor = BuildParameterEditor(parameter, provider, accessEvaluator, accessContext, editorRegistry);
    ...
    private static (FrameworkElement Row, Func<object?> Getter) BuildParameterEditor(
        InspectableMemberMetadata parameter,
        IObjectMetadataProvider provider,
        IMemberAccessEvaluator accessEvaluator,
        InspectableAccessContext accessContext,
        IInspectorEditorRegistry editorRegistry)
    {
        if (parameter.Kind == MemberKind.Class && typeof(ObservableObject).IsAssignableFrom(parameter.ValueType))
        {
            ...
            {
                var classControl = new ClassEditorControl(
                    $"{parameter.DisplayName} (param)",
                    instance,
                    provider,
