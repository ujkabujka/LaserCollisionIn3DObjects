...
public sealed class CollectionEditorControl : UserControl
{
    ...
    {
        var value = MemberAccess.GetValue(_member, _owner) as IEnumerable;
    ...
    private FrameworkElement CreateItemHost(object item)
    {
        if (item is ObservableObject nested)
        {
            ...
            var editor = new ClassEditorControl(
                nested.GetType().Name,
                nested,
                _provider,
                accessEvaluator: _accessEvaluator,
    ...
    private bool CanRemoveItems()
        => MemberAccess.CanWrite(_member) && MemberAccess.GetValue(_member, _owner) is IList;
    ...
    private void RemoveItem(object? item)
    {
        ...
        if (MemberAccess.GetValue(_member, _owner) is IList list && list.Contains(item))
        {
            list.Remove(item);
        }
    }
