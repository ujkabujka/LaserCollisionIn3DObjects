...
public sealed class ClassEditorControl : UserControl
{
    ...
    private readonly IMemberAccessEvaluator _accessEvaluator;
    private readonly InspectableAccessContext _accessContext;
    private readonly IInspectorEditorRegistry _editorRegistry;
    ...
    public ClassEditorControl(
        ...
        ObservableObject? owner = null,
        InspectableMemberMetadata? member = null,
        ...
        IInspectorEditorRegistry? editorRegistry = null)
    {
        ...
        _target = target;
        _provider = provider;
        _accessEvaluator = accessEvaluator ?? new DefaultMemberAccessEvaluator();
        _accessContext = accessContext ?? InspectableAccessContext.Empty;
        _editorRegistry = editorRegistry ?? new DefaultInspectorEditorRegistry();
        _owner = owner;
        _member = member;
        ...
        if (_owner is not null && _member is not null)
        {
            _typeOptions = BuildTypeOptions(_member.ValueType);
    ...
    {
        if (_member is null || string.IsNullOrEmpty(e.PropertyName))
        {
            return;
        }
    ...
    {
        if (_owner is null || _member is null)
        {
            return;
        }

        var newValue = MemberAccess.GetValue(_member, _owner) as ObservableObject;
        BindTarget(newValue);
    }

    private void HandleTypeSelectionChanged(object? sender, SelectionChangedEventArgs e)
    {
        if (_typeSelector?.SelectedValue is not Type selectedType || _member is null || _owner is null || !MemberAccess.CanWrite(_member))
        {
            return;
        }
        ...
        MemberAccess.SetValue(_member, _owner, instance);
        BindTarget(instance);
    }
    ...
    private void RefreshInspector()
    {
        ...
        _inspector.Bind(_target, _provider, _accessEvaluator, _accessContext, _editorRegistry);
