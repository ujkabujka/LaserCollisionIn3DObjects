...
public sealed class NoteEditorControl : UserControl
{
    ...
    {
        if (string.IsNullOrEmpty(e.PropertyName) || _member.Property is null)
        {
            return;
        }
    ...
    {
        if (_member.Property is null)
        {
            return;
        }

        var note = _member.Property.GetValue(_target) as NoteDocument ?? new NoteDocument();
        _isUpdating = true;
    ...
    private void Commit()
    {
        if (_isUpdating || _member.Property is null || MemberAccess.IsReadOnly(_member))
        {
            return;
        }
        ...
        var current = _member.Property.GetValue(_target) as NoteDocument;
        ...
        _member.Property.SetValue(_target, updated);
    }
}
